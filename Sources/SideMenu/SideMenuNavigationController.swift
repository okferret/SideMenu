...
@objc public enum SideMenuPushStyle: Int {
    ...
    internal var hidesBackButton: Bool {
        switch self {
        case .preserveAndHideBackButton: fallthrough
        case .replace: return true
        case .default: fallthrough
        case .popWhenPossible: fallthrough
        case .preserve: fallthrough
        case .subMenu: return false
        }
    }
}
...
/// SideMenuNavigationControllerTransitionDelegate
@MainActor
...
/// SideMenuSettings
@MainActor
public struct SideMenuSettings: Model {
    ...
    public init() {
        if Thread.isMainThread {
            MainActor.assumeIsolated {
                ...
                self.menuWidth = min(round(minimumSize * 0.75), 240)
            }
        }
    }
}
...
extension SideMenuNavigationController {
    ...
    internal func handleMenuPan(_ gesture: UIPanGestureRecognizer, _ presenting: Bool) {
        let width = menuWidth
    ...
    internal func cancelMenuPan(_ gesture: UIPanGestureRecognizer) {
        transitionController?.handle(state: .cancel)
    }
    
    internal func dismissMenu(animated flag: Bool = true, interactively: Bool = false, completion: (() -> Void)? = nil) {
    ...
    internal func present(from viewController: UIViewController?, interactively: Bool, completion: (() -> Void)? = nil) {
...
extension SideMenuNavigationController {
    ...
    private weak var activeDelegate: SideMenuNavigationControllerDelegate? {
    ...
    private func dismissAnimation(animated: Bool) {
        transitionController?.transition(presenting: false, animated: animated, alongsideTransition: { [weak self] in
            guard let self = self else { return }
            self.activeDelegate?.sideMenuWillDisappear?(menu: self, animated: animated)
    ...
    @discardableResult
    private func addSwipeToDismissGesture(to view: UIView?) -> UIPanGestureRecognizer? {
    ...
    @discardableResult
    private func addTapToDismissGesture(to view: UIView?) -> UITapGestureRecognizer? {
    ...
    @objc private func handleDismissMenuTap(_ tap: UITapGestureRecognizer) {
        let hitTest = view.window?.hitTest(tap.location(in: view.superview), with: nil)
        guard hitTest == view.superview else { return }
        dismissMenu()
    }
    
    @objc private func handleDismissMenuPan(_ gesture: UIPanGestureRecognizer) {
        handleMenuPan(gesture, false)
    }
    
    private func factor(_ presenting: Bool) -> CGFloat {
        return presenting ? presentFactor : hideFactor
    }
    
    private var presentFactor: CGFloat {
        return leftSide ? 1 : -1
    }
    
    private var hideFactor: CGFloat {
